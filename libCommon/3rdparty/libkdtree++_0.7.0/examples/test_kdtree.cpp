...
int main()
{
...

   {
      ...
      assert(found.first != exact_dist.end());
      std::cout << "Test find_nearest(), found at exact distance away from " << target << ", found " << *found.first << " @ " << found.second << " should be " << sqrt(8.0f) << std::endl;
      assert(found.second == sqrt(8.0f));
   }

   {
      ...
      std::pair<tree_type::const_iterator,double> found = exact_dist.find_nearest(target,sqrt(8.0f));
      assert(found.first != exact_dist.end());
      std::cout << "Test find_nearest(), found at exact distance away from " << target << ", found " << *found.first << " @ " << found.second << " should be " << sqrt(8.0f) << std::endl;
      assert(found.second == sqrt(8.0f));
   }
