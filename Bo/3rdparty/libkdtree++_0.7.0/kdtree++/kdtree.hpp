...
namespace KDTree
{
  ...
  template <size_t const __K, typename _Val,
	...
	\return a reference to the distance calculator used by the KDTree.
	  ...
	  // hz, 04.2010
	  typedef _Iterator<_Val, reference, pointer> mutable_iterator;
	  // end: hz
	  ...
	  // hz, 04.2010
	  mutable_iterator begin() { return mutable_iterator(_M_get_leftmost()); }
	  mutable_iterator end() { return mutable_iterator(static_cast<_Link_type>(&_M_header)); }
	  // end: hz
