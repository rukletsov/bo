...
namespace bo {
		   ...
		   _Predicate __p)
  {
    ...
    && (std::sqrt(_S_node_distance(probe_dim % __k, __dist, __acc, __val, static_cast<const _Node<_Val>* >(probe)->_M_value)) <= __max))
      {
	probe = near_node;
	++probe_dim;
      }
	...
	if (cur != __end)
	  {
	    ...
	    if (near_node
		// only visit node's children if node's plane intersect hypersphere
        && (std::sqrt(_S_node_distance(cur_dim % __k, __dist, __acc, __val, static_cast<const _Node<_Val>* >(cur)->_M_value)) <= __max))
	      {
		probe = near_node;
		++probe_dim;
	      }
